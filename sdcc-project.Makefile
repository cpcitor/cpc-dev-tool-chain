...
$(PROJNAME).ihx $(PROJNAME).map $(PROJNAME).noi: $(LOCALRELSFORCEDFIRST) $(RELS) $(LOCALRELSOTHERS) Makefile $(CDTC_ENV_FOR_SDCC) cdtc_project.conf $(LDLIBS)
	...
	. $(CDTC_ENV_FOR_SDCC) ; $(SDCC) -mz80 --no-std-crt0 -Wl-u $(LDFLAGS) $(LDLIBS) $(LOCALRELSFORCEDFIRST) $(filter crt0.rel,$(RELS)) $(filter-out crt0.rel,$(RELS)) $(LOCALRELSOTHERS) $${SDCC_LDFLAGS} -o "$@" ; )
