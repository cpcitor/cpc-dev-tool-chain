...
%.generated_from_asm_exported_symbols.h %.rel: %.s Makefile $(CDTC_ENV_FOR_SDCC) cdtc_project.conf
	...
	echo "// (2) even define some in reference to others," ; \
	echo "// (3) use them as parameters to macro calls," ; \
	echo "// (4) export them to C as... void pointers, not true constants." ; \
	echo ; \
	echo "// This file generated by cpc-dev-tool-chain allows you, " ; \
	echo "// *without any cost on the target*, to get" ; \
	echo "// C-level constants *guaranteed to be in sync with ASM*, " ; \
	...
	echo "// $<" ; \
	echo ; \
	sed -n 's|^S \([^\. ][^ ]*\) Def\([0-9A-F][0-9A-F][0-9A-F][0-9A-F]\)$$|#define ASMCONST_\1 0x\2|p' <"$${RELFILE}" ; \
	} >"$${OUTFILE}.tmp" \
	&& mv -vf "$${OUTFILE}.tmp" "$${OUTFILE}" ; \
	)
