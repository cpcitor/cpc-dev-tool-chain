...
$(PROJNAME).ihx $(PROJNAME).map $(PROJNAME).noi $(PROJNAME).lk: $(LOCALRELSFORCEDFIRST) $(RELS) $(LOCALRELSOTHERS) Makefile $(CDTC_ENV_FOR_SDCC) cdtc_project.conf $(LDLIBS)
	...
	set +xv ; if [[ "0x$$L__INITIALIZER" -ne 0 ]] ; then echo "WARNING: program uses global non-const variable initialized with compile-time constants, consuming $$(( 0x$${INITIALIZED} - 0x$${INITIALIZER} )) bytes twice.  It is good to do so, better than initializing them in code (main).  It is even better if the variables can be declared const: it will instantly divide memory consumption of those variables by 2.  Generally, declaring const whatever can be (including const pointer to const data) can yield shorter and faster code.  Please try to declare the symbols below const:" ; while read ADDRESS SYMBOL ; do [[ $$SYMBOL =~ ^s__ ]] && continue ; [[ 0x$$ADDRESS -lt 0x$$INITIALIZER ]] && continue ; [[ 0x$$ADDRESS -ge 0x$$INITIALIZED ]] && continue ; echo at $$ADDRESS symbol $$SYMBOL ; done < <( cat *.rst */*.rst | sed -n 's/^   \([0-9A-Fa-f][0-9A-Fa-f][0-9A-Fa-f][0-9A-Fa-f]\) *[0-9][0-9]* \([0-9a-zA-Z_]*\):$$/\1 \2/p' ) ; fi )
