...
* scalability: using a local macro forces to keep all elements on the same line, or with backslash continuation.
* convenience: backslash continuation are error-prone
* values are in a code file, which is not practical if values have to be generated.
...
enum
{
	color_count = sizeof(__dummy_colorname_count_t) / sizeof (XMACRO_COUNT_TYPE)
};
...
char *color_to_colorname(color color_)
{
